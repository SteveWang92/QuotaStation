...
/// The directories that actually hold Claude usage records, so a caller can watch them
/// for changes. Upstream only exposes `usage_files`, which walks the tree and returns
/// individual files; watching needs the containing directories instead.
pub fn claude_usage_paths() -> Result<Vec<PathBuf>> {
    Ok(claude_paths()?
        .into_iter()
